...
[ResumeApp](../Sources/Resume/ResumeApp.swift) composes one `@MainActor @Observable` [AppModel](../Sources/Resume/AppModel.swift). [ResumePanel](../Sources/Resume/ResumePanel.swift) presents Connection, library selection, Player, Search, Chapters and Settings in one fixed compact surface. Shared red styling and native controls cover ordinary and exceptional states. Settings is a panel mode routed by Command-comma and the app-menu command; there is no separate Settings scene. Temporary modes return to Player on Escape/dismissal, while Connection/library prerequisites remain reachable. Navigation does not change playing intent. Position recovery appears in the player only for an unresolved material conflict; retained alternatives are also available in Settings. The Debug-only UI host uses a native Window scene containing the same panel with fake dependencies, including the same Settings command routing. Its launch is suppressed unless `--ui-testing` is supplied; Release excludes it.
