...
Further amended by [issue #12](https://github.com/Grape9113/Resume/issues/12): Force Fetch and the synchronization dropdown are removed. Unresolved material conflicts expose inline recovery; retained alternatives remain in Settings. Choosing verifies the latest server position against the presented alternatives, then resolves authority without autoplay. If the server moved, refresh the choice rather than overwrite new progress. Subsequent writes remain automatic and guarded. This supersedes the earlier rule that selecting a recovery position itself leaves synchronization suspended indefinitely.
